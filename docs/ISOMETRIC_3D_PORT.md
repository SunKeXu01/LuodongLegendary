...
`v0.8.0` 从 JohanAR 更新、Oussama BOUKHELF 原作的 MIT `Trail3D`
实现迁移了相机朝向三角带的通用技术。项目版本将其重构成有界采样、按寿命
衰减的短时刀光，并接入青冥剑式、伏虎掌和敌方反击。机弩术仍使用方向明确
的直线曳光，因此近战与远程在 2.5D 镜头下能被快速区分。

只迁移通用拖尾算法，没有引入上游可见素材或游戏内容；固定版本、改动边界
和署名见 `THIRD_PARTY_NOTICES.md`。
