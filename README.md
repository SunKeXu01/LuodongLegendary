...
从 GitHub Releases 下载 `LuodongLegendary-0.11.0-win-x64-setup.exe`。安装向导采用 LZMA2 压缩，玩家可以修改安装目录，也可以选择是否创建桌面快捷方式。游戏仍然不需要联网。
...
`v0.10.0` 参照经典国产武侠页游的写实人物、青黑褐金主色和分层山水构图，
重做云津渡环境表现：河面使用动态 3D 波纹材质，商路加入错落青石，河岸
增加自然石带，方盒远山替换为多面山体与半透明山岚。左上角文字头像已经
替换为独立 SubViewport 实时渲染的 3D 少侠；HUD 面板统一使用鎏金角饰、
细边框和阴影层次。

`v0.11.0` 新增水墨风启动与角色创建界面，可输入原创角色名，并在青冥门、
沧浪门、锦衣行三种江湖出身中选择。出身会同步改变预览、场景角色和实时
头像服色，也会写入离线存档并显示于 HUD 和角色详情。云津渡地表与商道
